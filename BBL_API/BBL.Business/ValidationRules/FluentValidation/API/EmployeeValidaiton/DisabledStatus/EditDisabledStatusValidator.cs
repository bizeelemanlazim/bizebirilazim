...
{
    public class EditDisabledStatusValidator : AbstractValidator<EditDisabledStatusModel>
    {
        public EditDisabledStatusValidator()
        {
            RuleFor(x => x.Id).ExZoreControl();

            RuleFor(x => x.CategoryId).ExZoreControl();

            RuleFor(x => x.Percentage).ExZoreControl();
