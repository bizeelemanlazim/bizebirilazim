...
import {Colors, Global, Req} from '../../../util/Base';
...
function Ads({navigation}) {
  ...
  const [showMyActiveAds, setMyActiveAds] = useState(true);

  const getDataAsync = async () => {
    let res = await Req.GetAllAds({isActive: showMyActiveAds});
    setData(res.data)
  };
  ...
  //useEffect(getData, []);
  useEffect(getData, [showMyActiveAds]);
    ...
    <View style={{backgroundColor: Colors.BackgroundColor, flex: 1}}>
      ...
      <ScrollView>
        <View style={{paddingHorizontal: 20}}>
          <Breadcrumbs list={['Anasayfa', 'İlanlarım']} />
          <ShadowBox>
            <View style={{paddingHorizontal: 20}}>
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <Switch
                  ...
                  onValueChange={setMyActiveAds}
                  value={showMyActiveAds}
                />
                <Text style={{fontWeight: '500', fontSize: 13, color: 'black'}}>
                  {showMyActiveAds ? 'Aktif' : 'Pasif'} İlanlarım
                </Text>
                <View
                  style={{
              ...
              <CustomButton
                ...
                style={{marginTop: 10}}
                onPress={() => navigation.navigate('EmployerAdsAdd')}
              />
            </View>
            ...
            <FlatList
              ...
              keyExtractor={item => item.detailAdModel.jobId}
        ...
        <View style={{flex: 2, flexDirection: 'row', alignItems: 'center'}}>
          ...
          <View>
            <Text style={{fontSize: 13, color: 'black'}}>
              {item.detailOrderSummary.jobName}
            </Text>
            <Text style={{fontSize: 13, color: 'black'}}>Firma Ismi</Text>
          </View>
        </View>
        <View style={{flex: 0.7, alignItems: 'center'}}>
          <Text style={{color: 'black'}}>
            {item.detailOrderSummary.totalFees}
          </Text>
        </View>
        <View
          style={{
